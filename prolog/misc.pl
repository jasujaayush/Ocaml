...
available_at(X,Y) :- taqueria(Y, _, M), isin(X,M).

multi_available_helper(X) :- available_at(X,Y),available_at(X,Z), Y \= Z.
multi_available(A) :- bagof(Z, multi_available_helper(Z), Zs), remove_duplicates(Zs, Zdistinct), isin(A, Zdistinct).

working(X,Y) :- taqueria(Y, E, _), isin(X,E).
overworked_helper(X) :- working(X,Y),working(X,Z), Y \= Z.
overworked(X) :- bagof(Z, overworked_helper(Z), Zs), remove_duplicates(Zs,Zdistinct), isin(X, Zdistinct).

sum([], 0).
sum([H|T], C) :- sum(T, CT), cost(H, CH), C is  CT+CH.
total_cost(X,K) :- ingredients(X, I), sum(I, K). 

has_ingredients(X,[]) :- ingredients(X,_).
has_ingredients(X,[H|T]) :- ingredients(X, I), isin(H, I), has_ingredients(X,T).

is_ingredient(A) :- cost(A,_).
avoids_ingredients(X,[]) :- ingredients(X,_).
avoids_ingredients(X,[H|T]) :- ingredients(X, I), bagof(A, is_ingredient(A), Ingredients), isin(H,Ingredients), \+ isin(H, I), avoids_ingredients(X,T).
