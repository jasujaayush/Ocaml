...
let rec build (rand,depth) = 
	let x = rand (depth, depth+7) in (*Take a random number in the range, and based on its mod 7 take a branch*)
	match (x mod 7) with
	| 0 -> if depth = 0 then buildX() else buildSine(build(rand, depth - 1))
	| 1 -> if depth = 0 then buildY() else buildCosine(build(rand, depth - 1))
	| 2 -> if depth = 0 then buildSine(build(rand, depth)) else buildSine(build(rand, depth - 1))
	| 3 -> if depth = 0 then buildCosine(build(rand, depth)) else buildCosine(build(rand, depth - 1))
	| 4 -> if depth = 0 then build(rand, depth) else buildTimes(buildSine(build(rand, depth - 1)), buildCosine(build(rand, depth - 1)))
	| 5 -> if depth = 0 then build(rand, depth) else buildAverage(buildSine(build(rand, depth - 1)), buildCosine(build(rand, depth - 1)))
	| 6 -> if depth = 0 then build(rand, depth) else buildSine(build(rand, depth - 1))

let rec build2 (rand,depth) = 
	let x = rand (depth, depth+7) in (*Take a random number in the range, and based on its mod 9 take a branch*)
	match (x mod 9) with
	| 0 -> if depth = 0 then buildX() else buildTimes(buildX(), build(rand, depth - 1))
	| 1 -> if depth = 0 then buildY() else buildAverage(buildX(), build(rand, depth - 1))
	| 2 -> if depth = 0 then buildSine(build(rand, depth)) else buildTimes(buildSine(build(rand, depth - 1)), build(rand, depth - 1))
	| 3 -> if depth = 0 then buildCosine(build(rand, depth)) else buildAverage(buildCosine(build(rand, depth - 1)), build(rand, depth - 1))
	| 4 -> if depth = 0 then build(rand, depth) else buildAverage(buildCosine(build(rand, depth - 1)), build(rand, depth - 1))
	| 5 -> if depth = 0 then build(rand, depth) else buildTimes(build(rand, depth - 1), build(rand, depth - 1))
	| 6 -> if depth = 0 then build(rand, depth) else buildThresh(build(rand, depth - 1), build(rand, depth - 1), build(rand, depth - 1), build(rand, depth - 1))
  | 7 -> if depth = 0 then buildSigmoid(build(rand, depth)) else buildTimes(buildSigmoid(build(rand, depth - 1)), buildCosine(build(rand, depth - 1)))
  | 8 -> if depth = 0 then buildSigmoid(buildSine(build(rand, depth))) else buildSine(buildRoot(build(rand, depth - 1), build(rand, depth - 1), build(rand, depth - 1)))
